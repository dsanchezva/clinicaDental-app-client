...
function CreateTeam() {
  ...
  const handleImage = async (e) =>{
    ...
    uploadData.append("teamPicture", e.target.files[0]);
    try {
      const response = await service.patch('/team/uploadTeamPic', uploadData)
      setImage(response.data.teamPic)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await service.post('/team/create', data)
      navigate('/team')
